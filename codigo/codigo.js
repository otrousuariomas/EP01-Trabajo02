    ...
    */
    var validaRespuesta = function(id)
    {
        ...
        for(var i = 0; i < mathSquare.length - 1; i++)
        {
            if(i % 2 === 0)
        	  {
        		    Roperacion.horizontal.ecuacion += nom_div(partId[0]+"_"+i).value.length !== 0 ? nom_div(partId[0]+"_"+i).value : 0;
        		    Roperacion.vertical.ecuacion += nom_div(i+"_"+partId[1]).value.length !== 0 ? nom_div(i+"_"+partId[1]).value : 0;
        	  }
        	  else
        	  {
        		    Roperacion.horizontal.ecuacion += mathSquare[Number(partId[0])][i];
        		    Roperacion.vertical.ecuacion += mathSquare[i][Number(partId[1])];
        	  }
        }

        if (eval(Roperacion.horizontal.ecuacion) === Matriz.horizontal)
        {
            nom_div(Roperacion.horizontal.div).style.background="green";
        }
        else
        {
            nom_div(Roperacion.horizontal.div).style.background="red";
        }
        if (eval(Roperacion.vertical.ecuacion) === Matriz.vertical)
        {
            nom_div(Roperacion.vertical.div).style.background="green";
        }
        else
        {
            nom_div(Roperacion.vertical.div).style.background="red";
        }
    };
